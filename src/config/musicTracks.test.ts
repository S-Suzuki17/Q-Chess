...
describe('reward recordings',()=>{
 ...
 it('preserves existing unlock IDs while replacing all remaining recordings',()=>{
  ...
  expect(battleMusicTitle('astral')).toBe('盤上の幾何学');
  expect(battleMusicTitle('zenith')).toBe('Twelve Moves Ahead');
  expect(battleMusicTitle('valkyrie')).toBe('The Eighth Rank');
  expect(REWARD_TRACKS.filter(track=>track.url.endsWith('.mp3'))).toHaveLength(15);
  expect(REWARD_TRACKS.filter(track=>track.url.endsWith('.wav'))).toHaveLength(0);
 });
});
