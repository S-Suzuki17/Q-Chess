...
export function AccountAvatar({name,url,frame,size=64,rating,lang='en'}:{name:string;url?:string;frame?:string;size?:number;rating?:number|null;lang?:string}) {
    ...
    return <span className="account-avatar" style={{width:size,height:size}} data-avatar-frame={decoration?.id??'standard'}>
        ...
        {decoration&&<svg viewBox="0 0 120 120" aria-hidden="true" className="account-avatar-frame" fill="none" style={{color:decoration.color}}>
            ...
            {decoration.motif==='crown'&&<><path d="M31 15 25 -2 45 9 60 -7 75 9 95 -2 89 15Z" fill="currentColor" stroke={decoration.accent}/><path d="M27 104q33 22 66 0l-6 14H33Z" fill="currentColor"/></>}
