...
import { ActiveMatch, getActiveMatches, Profile, getProfile } from '../lib/gameRecordService';
    ...
    const loadMatches = async () => {
        ...
        for (const m of data) {
            if (m.white_id && !profileMap[m.white_id]) {
                const p = await getProfile(m.white_id);
                if (p) profileMap[m.white_id] = p;
            }
            if (m.black_id && !profileMap[m.black_id]) {
                const p = await getProfile(m.black_id);
            ...
            <div className="bg-[#11100E] border border-[#B39A62]/30 p-6 rounded-lg max-w-md w-full shadow-2xl max-h-[80vh] overflow-y-auto">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-2xl font-bold text-[#E8E2D7] font-serif tracking-widest flex items-center gap-2">
                        ...
                        {matchText(lang, "対局中の試合", "Live Matches")}
                    </h3>
                    <button onClick={onClose} className="text-[#A89C86] hover:text-[#E8E2D7]">✕</button>
                </div>

                {loading && matches.length === 0 ? (
                    <p className="text-[#A89C86] text-sm text-center py-4">{matchText(lang, "対局を読み込み中…", "Loading active matches...")}</p>
                ) : matches.length === 0 ? (
                    <p className="text-gray-600 text-sm text-center py-4">{matchText(lang, "現在対局中の試合はありません", "No active matches at the moment.")}</p>
                ) : (
                            ...
                            return (
                                ...
                                >
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-[10px] text-[#A89C86] mt-1">{new Intl.NumberFormat(lang,{style:'unit',unit:'minute',unitDisplay:'short'}).format(duration)}</span>
                                    </div>
                                    <span className="text-red-400 font-bold text-xs">
                                        {matchText(lang, "観戦", "Spectate")}
                                    </span>
                                </button>
                            );
                        })}
