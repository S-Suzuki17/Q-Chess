...
export function ChampionshipCollection({lang,progress,onEquip}:{lang:Language;progress:CampaignProgress;onEquip:(kind:'board'|'effect'|'piece'|'music'|'avatar',id:string)=>void}) {
    ...
    return <section className="championship-collection" aria-label={t('collection')}>
        ...
        <div className="championship-grid">{CHAMPIONSHIP_REWARDS.slice((grade-1)*10,grade*10).map(reward=>{
            ...
            return <article key={reward.id} className="championship-reward" data-championship-reward={reward.id} data-piece-form={reward.kind==='piece'?reward.form:undefined} data-board-profile={reward.kind==='board'?reward.profile:undefined} data-reward-grade={reward.tier} data-equipped={equipped}>
                <span className="championship-number"><span>{String(reward.requiredWins).padStart(3,'0')} <span className="championship-edition">/ 100</span></span>{equipped ? <Check size={15}/> : !unlocked ? <LockKeyhole size={13}/> : null}</span>
                {reward.kind==='music'?<div className="reward-art-stage"><MusicRewardArtwork tier={reward.tier}/></div>:<button className="reward-art-stage" aria-label={`${circuitText(lang,'preview')} · ${rewardName(lang,reward.id)}`} onClick={()=>setPreview({kind:reward.kind,id:reward.id})}>
                    {reward.kind==='board'?<BoardRewardArtwork preset={reward}/>:reward.kind==='effect'?<EffectRewardArtwork preset={reward}/>:reward.kind==='avatar'?<AccountAvatar name="Q" frame={reward.id} size={100}/>:<PieceRewardArtwork finish={reward.id} tier={reward.tier}/>}
                </button>}
                ...
                <span className="championship-unlock">{equipped?campaignText(lang,'equipped'):unlocked?campaignText(lang,'equip'):`${stageText(lang,'stage')} ${reward.requiredWins}`}</span>
                <div className="championship-reward-actions">{reward.kind!=='music'&&<button data-preview-reward={reward.id} onClick={()=>setPreview({kind:reward.kind,id:reward.id})}>{circuitText(lang,'preview')}</button>}
                    <button data-equip-reward={reward.id} disabled={!unlocked} aria-pressed={equipped} onClick={()=>onEquip(reward.kind,reward.id)}>{campaignText(lang,equipped?'equipped':unlocked?'equip':'locked')}</button></div>
            </article>;
        })}</div>
        ...
        {ARCHIVED_REWARDS.some(reward=>rewardUnlocked(progress,reward.id))&&<details className="campaign-legacy-collection"><summary>{campaignText(lang,'rewards')} · {campaignText(lang,'cleared')}</summary><div className="campaign-equipment">{ARCHIVED_REWARDS.filter(reward=>rewardUnlocked(progress,reward.id)).map(reward=><button key={reward.id} aria-pressed={progress[reward.kind]===reward.id} onClick={()=>onEquip(reward.kind,reward.id)}>{rewardName(lang,reward.id)}</button>)}</div></details>}
        <div className="championship-effect-actions"><button aria-pressed={!progress.avatar||progress.avatar==='standard'} onClick={()=>onEquip('avatar','standard')}>{stageText(lang,'frame')} · {campaignText(lang,'standard')}</button><button className="championship-default-effect" aria-pressed={progress.effect==='standard'} onClick={()=>onEquip('effect','standard')}>{t('effect')} · {campaignText(lang,'standard')}</button>
        ...
        {preview && <RewardPreview lang={lang} reward={preview} progress={progress} onClose={()=>setPreview(null)} onEquip={onEquip}/>}
    </section>;
}
